...
BuildRequires: python%{pyver}-hacking >= 1.1.0-3
BuildRequires: python%{pyver}-coverage >= 4.4.2
BuildRequires: python%{pyver}-flake8-import-order = 0.18
...
BuildRequires: python%{pyver}-doc8 >= 0.8.2
BuildRequires: python%{pyver}-oslotest >= 3.2.0
BuildRequires: python%{pyver}-os-testr >= 1.0.0
%if 0%{?fedora} || 0%{?rhel} > 7
BuildRequires: python3-pylint == 2.3.1-1
%else
BuildRequires: python2-pylint == 1.9.5-1
%endif
BuildRequires: python%{pyver}-testresources >= 2.0.1-1
BuildRequires: python%{pyver}-testscenarios >= 0.4
BuildRequires: python%{pyver}-webtest >= 2.0.30-1

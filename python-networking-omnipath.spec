...
License:        ASL 2.0
URL:            https://opendev.org/x/networking-omnipath.git
...
%package -n     python%{pyver}-networking-%{sname}
Summary:        %{summary}
%{?python_provide:%python_provide python%{pyver}-networking-%{sname}}
...
%description -n python%{pyver}-networking-%{sname}
OpenStack networking-omnipath is a ML2 mechanism driver that integrates
OpenStack Neutron API with Omnipath backend. It enables Omnipath switching
fabric in OpenStack cloud and each network in Openstack networking realm
corresponds to a virtual fabric on omnipath side.

#%package -n python%{pyver}-networking-%{sname}-tests
...
#Requires: python%{pyver}-networking-%{sname} = %{version}-%{release}
#Requires: python%{pyver}-jsonschema
